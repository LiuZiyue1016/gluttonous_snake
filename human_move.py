...
class Game:
    ...
    def game_end(self, t):
        ...
        if ((self.snake.position[0] >= self.settings.width or self.snake.position[0] < 0
                or self.snake.position[1] >= self.settings.height or self.snake.position[1] < 0)
                or self.snake.segments[0] in self.snake.segments[1:]):
            end = True
            self.update_record(t)
        return end
    ...
    def update_record(self, t_):
        record = []
        score = self.snake.score
        try:
            with open(self.record_name, "r") as file:
                lines = file.readlines()
                for line in lines:
                    t, s = line.strip().split(",")
                    record.append((float(t), int(s)))
        except FileNotFoundError:
            with open(self.record_name, "w") as file:
                file.write(f"{t_},{score}\n")
                record.append((float(t_), int(score)))

        # Check if the t already exists in the record
        time_exists = False
        for i, (t, s) in enumerate(record):
            if t_ == t:
                record[i] = (t, score)  # Update the score
                time_exists = True
